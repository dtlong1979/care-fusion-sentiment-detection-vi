...
p("Chúng tôi nghiên cứu vai trò của biểu tượng cảm xúc (emoji/emoticon) trong phân loại cảm xúc "
  ...
  "ở mức transformer qua 3 bộ dữ liệu / 2 ngôn ngữ (PhoBERT cho ViGoEmotions & UIT-VSMEC; XLM-R cho "
  "GoEmotions) cho thấy định tuyến theo chế độ KHÔNG vượt baseline mạnh nhất (text-only/cross-attention) "
  "một cách có ý nghĩa thống kê; CARE đạt trung bình tốt nhất trên bộ GIÀU EMOJI nhất (UIT-VSMEC, 0.604) "
  "— gợi ý lợi ích tỉ lệ với mật độ emoji. Do đó chúng tôi định vị đóng góp theo hướng phân tích/tài "
  "nguyên + tính giải thích được. Một phát hiện trọng tâm: "
...
h2("6.10 Kết quả chính thức: transformer × 3 dataset (5 seed)")
p("Chạy thật trên A100: PhoBERT-base cho hai bộ tiếng Việt (ViGoEmotions, UIT-VSMEC), XLM-R-base cho "
  "GoEmotions (tiếng Anh); 5 seed; macro-F1 mean±std trên test.")
p("Bảng 8. Macro-F1 (test, 5 seed) ở mức transformer qua 3 bộ dữ liệu.", ("", "i"))
table(["Mô hình", "ViGoEmotions (PhoBERT)", "UIT-VSMEC (PhoBERT)", "GoEmotions (XLM-R)"],
      [["B0 majority", "0.111", "0.092", "0.124"],
       ["B1 text-only", "0.466 ± 0.016", "0.577 ± 0.022", "0.465 ± 0.040"],
       ["B2 marker-concat", "0.491 ± 0.010", "0.544 ± 0.026", "0.473 ± 0.057"],
       ["B3 scalar-gated", "0.496 ± 0.012", "0.544 ± 0.017", "0.464 ± 0.045"],
       ["B4 cross-attn", "0.509 ± 0.025", "0.569 ± 0.024", "0.427 ± 0.038"],
       ["CARE-Fusion (full)", "0.488 ± 0.017", "0.604 ± 0.036", "0.489 ± 0.074"]])
p("(Ablation CARE trên ViGoEmotions: −routing 0.499, −delta 0.513, −cf 0.501, −intensity 0.502, "
  "−gcn 0.499, −confidence 0.512 — đều trong khoảng nhiễu của CARE_full.)")
p(("Kiểm định (Wilcoxon, 5 seed). ", "b"),
  "Lưu ý: với 5 seed, p nhỏ nhất khả dĩ của Wilcoxon là 0.0625 (cả 5 seed cùng chiều); p<0.05 cần ≥6 seed. "
  "(i) ViGoEmotions: CARE_full KHÔNG vượt mô hình nào (vs B4 Δ=−0.021 p=0.44; vs B1 p=0.19). "
  "(ii) UIT-VSMEC (giàu emoji 34%): CARE_full CAO NHẤT (0.604), thắng B2/B3 ở TOÀN BỘ 5 seed "
  "(Δ≈+0.060, p=0.0625) nhưng so với B1 text-only/B4 cross-attn thì KHÔNG có ý nghĩa (p=0.31). "
  "(iii) GoEmotions: CARE_full cao nhất danh nghĩa (0.489) nhưng std=0.074 quá lớn → không ý nghĩa.")
p(("Kết luận: ", "b"), "trên cả 3 bộ, CARE-Fusion KHÔNG vượt baseline mạnh nhất (text-only/cross-attn) "
  "một cách có ý nghĩa thống kê. CARE đạt trung bình tốt nhất trên bộ GIÀU EMOJI (VSMEC) và thắng các "
  "baseline hợp nhất đơn giản (concat/gated) ở toàn bộ 5 seed tại đó — gợi ý lợi ích của hợp nhất có "
  "cấu trúc TỈ LỆ với mật độ emoji, nhưng chưa đủ mạnh để khẳng định. Biểu tượng cảm xúc cải thiện rõ "
  "so với chỉ-text (đặc biệt VSMEC: 0.577→0.604), song hợp nhất đơn giản nhìn chung đã đủ.")
